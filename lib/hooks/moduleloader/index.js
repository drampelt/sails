  ...
  return {
      ...
      async.auto({
        ...
        'config/*': function loadOtherConfigFiles (cb) {
          ...
          buildDictionary.aggregate({
            ...
            filter    : /(.+)\.(js|json|coffee|litcoffee|iced|liticed|iced.md)$/,
        ...
        'config/local' : function loadLocalOverrideFile (cb) {
          buildDictionary.aggregate({
            dirname   : sails.config.paths.config || sails.config.appPath + '/config',
            filter    : /local\.(js|json|coffee|litcoffee|iced|liticed|iced.md)$/,
            identity  : false
          }, cb);
        },
        ...
        'config/env/*' : ['config/local', function loadEnvConfigFile (cb, async_data) {
          ...
          buildDictionary.aggregate({
            ...
            filter    : new RegExp(env + '.(js|json|coffee|litcoffee|iced|liticed|iced.md)$'),
            optional  : true,
            flattenDirectories: !(sails.config.dontFlattenConfig),
            identity  : false
          }, cb);
    ...
    loadControllers: function (cb) {
      buildDictionary.optional({
        ...
        filter: /(.+)Controller\.(js|coffee|litcoffee|iced|liticed|iced.md)$/,
    ...
    loadAdapters: function (cb) {
      buildDictionary.optional({
        ...
        filter    : /(.+Adapter)\.(js|coffee|litcoffee|iced|liticed|iced.md)$/,
        replaceExpr : /Adapter/,
        flattenDirectories: true
      }, bindToSails(cb));
    },
    ...
    loadModels: function (cb) {
      ...
      buildDictionary.optional({
        ...
        filter    : /^([^.]+)\.(js|coffee|litcoffee|iced|liticed|iced.md)$/,
        replaceExpr : /^.*\//,
        flattenDirectories: true
      }, function(err, models) {
        if (err) {return cb(err);}
